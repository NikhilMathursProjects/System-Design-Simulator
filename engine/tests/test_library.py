...
from sysdesign_engine.library.loader import DEFAULT_LIBRARY_PATH, LibraryError, load_library
...
#--------------------------------------helpers: a minimal valid entry per capability, used as a mutation base----------------------------------------------------
...
#----------------------------------one valid construction per capability----------------------------------



def test_valid_compute():
    e = make(type="server", caps=["compute"],
              service_times={"compute": {"p50": 1.0, "p99": 2.0}})
    assert e.caps == ["compute"]


def test_valid_route():
    e = make(type="lb", caps=["route"],
              service_times={"forward": {"p50": 0.3, "p99": 1.0}})
    assert e.caps == ["route"]


def test_valid_store_persistent():
    e = make(type="db", caps=["store"], persistent=True,
...
def test_valid_store_volatile():
    e = make(type="cache", caps=["store"], persistent=False,
              service_times={"read": {"p50": 1.0, "p99": 2.0},
                              "write": {"p50": 1.0, "p99": 2.0}},
...
def test_valid_multi_cap_route_and_store():
    e = make(type="edge_cache", caps=["route", "store"], persistent=False,
              service_times={"forward": {"p50": 0.5, "p99": 1.0},
...
def test_error_persistent_on_non_store():
    with pytest.raises(ValidationError):
        make(type="server", caps=["compute"], persistent=True,
             service_times={"compute": {"p50": 1.0, "p99": 2.0}})


def test_error_volatile_without_per_gb_hour():
    with pytest.raises(ValidationError):
        make(type="cache", caps=["store"], persistent=False,
             service_times={"read": {"p50": 1.0, "p99": 2.0},
                             "write": {"p50": 1.0, "p99": 2.0}},
...
def test_error_cap_missing_service_op():
    with pytest.raises(ValidationError):
        make(type="db", caps=["store"], persistent=True,
             service_times={"read": {"p50": 1.0, "p99": 2.0}})
...
def test_loader_wraps_validation_error_readably(tmp_path):
    ...
    bad["service_times"] = {"compute": {"p50": 5.0, "p99": 1.0}}  # p50 >= p99
...
def test_relative_physics_read_latency_ordering():
    lib = load_library()
    redis_read = lib.get("redis").service_times.read.p50
